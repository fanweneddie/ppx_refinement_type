...
let ctx_lookup (ctx: rty_ctx) (ident: string): (string * string * rty) option =
  List.find_opt (fun (name, prefix, _) -> String.equal (prefix ^ name) ident) ctx
...
let entailment (ctx: full_ctx) (pred: Expr.expr): Expr.expr =
  List.fold_left
    (fun pred (name, prefix, ty) ->
      match ty with
      | RtyBase {base_ty; phi} ->
        print_endline "create_var1";
        let v = Smtcheck.create_var ctx.z3 ctx.ctr prefix "v" base_ty in
        let x = Smtcheck.create_var ctx.z3 ctx.ctr prefix ("var_" ^ prefix ^ name) base_ty in
        print_endline "create_var1 done";
...
let rec type_infer (ctx: full_ctx) (e: Typedtree.expression) : rty =
  ...
  | Texp_let(_, [vb], expr) -> 
    ...
    | Tpat_var(ident, _) ->
      ...
      let new_ctx = {ctx with rty = (name, ctx.prefix, rty1)::ctx.rty} in
  ...
  | Texp_function{param; cases = [{c_rhs; _}]; _} ->
    ...
    | Tarrow (_, arg_ty, _, _) ->
      ...
      let new_ctx = {ctx with rty = (Ident.name param, ctx.prefix, arg_rty)::ctx.rty} in
      let ret_rty = type_infer new_ctx c_rhs in
      RtyArrow {arg_name; arg_rty; ret_rty}
  ...
  | Texp_apply(op, args) ->
    ...
    (match ty with
    | RtyBase(_) -> RtyBase { base_ty = e.exp_type; phi = Boolean.mk_true ctx.z3 }
    | RtyArrow(_) ->
      let arg_exprs = 
        List.map 
          (fun arg -> 
  ...
  | Texp_construct(_, {cstr_name; _}, _) ->
    ...
    | _ -> RtyBase { base_ty = e.exp_type; phi = Boolean.mk_true ctx.z3 })
  | Texp_variant(_)
  | Texp_record(_)
  | Texp_field(_)
  | Texp_setfield(_)
...
and type_check (ctx: full_ctx) (e: Typedtree.expression) (ty: rty): unit =
  ...
  | Texp_ident (path, _, _) ->
    ...
    let (pref, ty') = 
      (match ctx_lookup (ctx.rty (*@ ctx.curr*)) name with
      | None -> (ctx.prefix, type_infer ctx e)
      | Some (_, pref, ty') -> (pref, ty'))
    in
    (* Check if this is correct *)
    print_endline "ident";
    print_endline name;
    check_subtype e.exp_env ctx ("var_" ^ name, pref, ty') ("v", ctx.prefix, ty)
  | Texp_apply(_)
  | Texp_constant(_) ->
    let ty' = type_infer ctx e in
    check_subtype e.exp_env ctx ("v", ctx.prefix, ty') ("v", ctx.prefix, ty)
  | Texp_function {param; cases = [{c_rhs; _}]; _} ->
    (match ty with
...
let type_item (ctx: full_ctx) (prefix: string) (item: Typedtree.structure_item) : full_ctx =
  ...
  | Tstr_value (_, [vb]) ->
    ...
    | None ->
      ...
      { ctx with rty = (name, prefix, rty)::ctx.rty }
